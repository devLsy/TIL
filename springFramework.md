...
## 단위테스트 jUnit
<strong>단위테스트 하기 위해 매번 Tomcat을 실행하지 않고도 jUnit 테스트 도구를 이용해서 Tomcat 실행 없이 springFramework를 구동시켜서 DB Connection, CRUD등의 테스트가 가능함, 매우 효율적이므로 개발 시에 항상 작성하는 습관을 기르도록 하자, 스프링5 버전의 경우 pom.xml에서 jUnit 버전을 4.12 이상으로 해야 한다고 함,<strong> 아래는 사용 예제 이미지임<br>
